...
- 한국어 단어 게임은 현재 도박·확률형 유료 아이템·위치정보가 없지만, opt-in 전체 사전에 민감 텍스트가 있으므로 연령 등급을 미리 단정하지 않는다. IARC 설문에 실제 콘텐츠를 답하고 한국 배포 등급 결과에 따라 GRAC 관련 조치를 다시 확인한다.
...
- [ ] **Content rating:** IARC 설문에 종료 뒤 opt-in으로 접근 가능한 성인·비속어·폭력·약물 관련 사전 단어를 실제 텍스트 콘텐츠로 신고한다. 기본 숨김이나 경고를 이유로 `없음`이라고 답하지 않는다.
- [ ] **Target audience:** 초기에는 `16–17`, `18+`만 선택한다. 15세 이하로 확대하려면 정답뿐 아니라 추측·전체 순위 노출까지 다시 검수하고 IARC 결과와 일치시킨다.
...
- [ ] 민감 텍스트 opt-in을 포함해 IARC가 산출한 한국 배포 등급을 확인한다. 19세 이용가 결과나 모의 도박·확률형 유료 아이템이 생기면 GRAC와 확률 공개 요구를 별도로 검토한다.
