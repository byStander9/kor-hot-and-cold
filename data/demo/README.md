...
- 문제 시드: 0~4,294,967,295(약 42억 개)
- 정답 후보: 전체 표제어 280,804개
...
`vectors.json`은 임베딩 차원, 모델 버전과 분할 파일 목록을 기록합니다. `vectors-*.bin`은 정규화 임베딩을 행 우선 `int8`로 양자화한 파일입니다. 서버가 시드로 정답을 고른 뒤 전체 벡터와 내적해 순위를 동적으로 계산하므로 정답마다 별도 퍼즐 JSON을 만들 필요가 없습니다. 동일 시드의 결과는 데이터 버전 1 안에서 고정됩니다.
